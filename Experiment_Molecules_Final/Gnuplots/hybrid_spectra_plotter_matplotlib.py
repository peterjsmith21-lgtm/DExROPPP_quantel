...
max_intensity = max(np.max(broad_singlet), np.max(broad_triplet))
print(f"Singlet max intensity: {np.max(broad_singlet):.4f}")
print(f"Triplet max intensity: {np.max(broad_triplet):.4f}")

# Normalise combined spectrum
if np.max(broad_singlet) > 0:
    broad_singlet = broad_singlet / max_intensity
if np.max(broad_triplet) > 0:
    broad_triplet = broad_triplet / max_intensity
