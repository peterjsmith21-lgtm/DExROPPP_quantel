...
def ci_rot(ndocc,norbs,coords,atoms,energy0,repulsion,orb_energies,hf_orbs, file, ci_level):
    ...
    with open(output_path,'w') as out:
        ...
        if ci_level < 2:
            ...
            ham_rot, singlet_block, triplet_block, quintet_block = get_full_CIMatrix(ndocc, norbs, energy0, orb_energies, rep_tens, ci_level)
    ...
    for i in range(dens_mo.shape[0]):
        ...
        for j in range(dens_mo.shape[0]):
            if i!=j and fock_mo[i,j] > 1e-4:
                ...
                print(dens_mo)
                sys.exit()
    strngs, ci_energies_array, osc_arrays, s2_array = ci_rot(ndocc, natoms, coord, atoms_array, energy0, two_body, orb_energy, hf_orbs, file, ci_level=2)
